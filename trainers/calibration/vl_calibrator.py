...
# testadd
from trainers.calibration.visual_informed_calibrator import VisualInformedCalibration
# middleadd
from trainers.calibration.middle_model_calibrator import MiddleModelCalibration
...
class VLCalibration():
    ...
    def __init__(self, cfg, base_calibration_mode=None, base_bin_calibrator_name=None, dac_flag=False, procal_flag=False, vi_flag=False, middle_flag=False, val_dict=None, text_feature_dict=None, middle_text_feature_dict=None, if_test=False):
        """
        ...
        Args:
            ...
            vi_flag (bool): Flag indicating if visual-informed methods are used.
            middle_flag (bool): Flag indicating if middle-model methods are used.
            val_dict (dict): Dictionary containing validation set data like logits, image and text features, labels, etc.
            middle_text_feature_dict (dict): Dictionary containing text feature of base class and current test class using middle tuned model
        ...
        # testadd:
        self.vi_flag = vi_flag
        # middleadd:
        self.middle_flag = middle_flag
        ...
        if middle_text_feature_dict is not None:
            print('VLCalibration处使用middle_text_feature_dict')
            self.base_text_features_middle = middle_text_feature_dict['base_text_features_middle']
            self.current_text_features_middle = middle_text_feature_dict['current_text_features_middle']
    ...
    def fit(self):
        ...
        # testadd:
        self.vi_calibrator = None
        # middleadd:
        self.middle_calibrator = None
        ...
        # testadd
        if self.vi_flag:
            self.vi_calibrator = self.build_vi_calibrator(self.val_image_features, self.val_text_features)
            
        # middleadd
        if self.middle_flag:
            self.middle_calibrator = self.build_middle_calibrator(self.text_feature_dict, self.base_text_features_middle, self.current_text_features_middle)
    ...
    def predict(self, logits, test_proximity, image_features_test, text_features_test):
        ...
        # testadd:
        if self.vi_calibrator is not None:
            logits = self.vi_calibrator.predict(logits, image_features_test, text_features_test, self.if_test)
            
        # middleadd:
        if self.middle_calibrator is not None:
            logits = self.middle_calibrator.predict(logits)
    ...
    # testadd
    def build_vi_calibrator(self, val_image_features, val_text_features):
        vi_calibrator = VisualInformedCalibration()
        vi_calibrator.fit(val_image_features, val_text_features)
        return vi_calibrator
    
    # middleadd
    def build_middle_calibrator(self, text_feature_dict, base_text_features_middle, current_text_features_middle):
        middle_calibrator = MiddleModelCalibration()
        middle_calibrator.fit(text_feature_dict, base_text_features_middle, current_text_features_middle)
        return middle_calibrator
