...
def reset_cfg(cfg, args):
    ...
    if args.calibration_config:
        ...
        # testadd: reset_cfg处
        if calibration_cfgs['IF_VISUAL_INFORMED']:
            cfg.CALIBRATION.VISUAL_INFORMED.IF_VISUAL_INFORMED = calibration_cfgs['IF_VISUAL_INFORMED']

        # middleadd: reset_cfg处
        if calibration_cfgs['IF_MIDDLE_MODEL']:
            cfg.CALIBRATION.MIDDLE_MODEL.IF_MIDDLE_MODEL = calibration_cfgs['IF_MIDDLE_MODEL']
...
def extend_cfg(cfg):
    ...
    # question01: 有些矛盾，有的地方CoCoOp的N_CTX写的是16，有的是4
    # 问题不大，之后从config里面导入的会覆盖，正确的应该是4
    ...
    # testadd: Config for visual informed calibration
    cfg.CALIBRATION.VISUAL_INFORMED = CN()
    cfg.CALIBRATION.VISUAL_INFORMED.IF_VISUAL_INFORMED = False

    # middleadd: Config for middle model calibration
    cfg.CALIBRATION.MIDDLE_MODEL = CN()
    cfg.CALIBRATION.MIDDLE_MODEL.IF_MIDDLE_MODEL = False
...
def setup_cfg(args):
    ...
    # question02: 为什么这里是dataset_config_file，之前add_argument写的是dataset-config-file?
    # 命令行参数命名规范：
    # 1. 在命令行中，参数通常使用连字符（-）分隔，以便于阅读和符合惯例。例如，--dataset-config-file。
    # 2. 在 Python 代码中，变量名通常使用下划线（_）分隔，以符合 Python 的命名规范。例如，dataset_config_file。
    # argparse 模块的行为：
    # 1. argparse 模块会自动将命令行参数中的连字符转换为下划线，以便与 Python 变量名保持一致。
    # 2. 因此，当你在命令行中使用 --dataset-config-file 时，argparse 会将其转换为 dataset_config_file。
    ...
    reset_cfg(cfg, args)   # VISUAL_INFORMED需要修改处
...
def main(args):
    ...
    # testadd:
    if cfg.CALIBRATION.VISUAL_INFORMED.IF_VISUAL_INFORMED:
        base_name = base_name + '_vi'

    # middleadd:
    if cfg.CALIBRATION.MIDDLE_MODEL.IF_MIDDLE_MODEL:
        base_name = base_name + '_middle'
