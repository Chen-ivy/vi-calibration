...
# TRAINERS=('KgCoOp' 'MaPLe' 'PromptSRC')
...
VISUAL_INFORMED=true # true, false
MIDDLE_MODEL=false # true, false
...
for TRAINER in "${TRAINERS[@]}"; do
    ...
    if [ "${CALIBRATION}" == "None" ]; then
        cal_cfgs='{
            ...
            "IF_DAC": '${DAC}',
            "IF_PROCAL": '${PROCAL}',
            "IF_VISUAL_INFORMED": '${VISUAL_INFORMED}',
            "IF_MIDDLE_MODEL": '${MIDDLE_MODEL}'
        }'
    else
        cal_cfgs='{
            "BASE_CALIBRATION_MODE": "scaling_based",
